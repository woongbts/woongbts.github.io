...
new='''            if price is None or price <= 0 or release in ("", "0000-00-00"):\n                continue\n            # The source list is already RELEASE_DT_DESC. Keep only recent releases so\n            # discontinued/legacy devices cannot leak back into calculators or recommendations.\n            if release < RECENT_DEVICE_CUTOFF:\n                continue\n            candidates.append((order, device))\n'''
assert s.count(old)==1, s.count(old)
s=s.replace(old,new,1)
...
# 2) Update client recommendation behavior to match the recent-device catalog.
...
# Remove the legacy StyleFolder2 forced recommendation record.
...
# Senior ordering: newest current value models first, older A17 only as a later recent option.
...
new='const c=(o?.devices||[]).filter(t=>t.carrier===e&&V(t,a)&&n(t.retail_price)&&Number(t.retail_price)>0&&!xe(t)&&!Ce(t)&&!/폴더|folder/i.test(`${t.name||""} ${t.model||""} ${t.model_code||""}`)).sort(m).slice(0,140),s=[];'
assert j.count(old)==1, j.count(old)
j=j.replace(old,new,1)

# Bust caches for refreshed public wireless data.
for old,new in [
