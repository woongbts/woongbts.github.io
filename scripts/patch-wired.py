...
old="let internetData={providers:[],internet_products:[],tv_products:[],bundle_rules:[]};"
new="""let internetData={providers:[],internet_products:[],tv_products:[],settop_products:[],bundle_rules:[]};
...
if old not in s:
    raise SystemExit('internetData marker not found')
s=s.replace(old,new,1)

old="""  function fillBundleSelect(select,kind){
...
"""
new="""  function fillBundleSelect(select,kind){
...
"""
if old not in s:
    raise SystemExit('bundle block marker not found')
s=s.replace(old,new,1)

start=s.index('  function syncInternet(){')
end=s.index("  internetCarrier.addEventListener('change',fillInternetProducts);",start)
...
s=s.replace("tvProduct.addEventListener('change',syncInternet);","tvProduct.addEventListener('change',fillInternetBundles);",1)
p.write_text(s,encoding='utf-8')

h=Path('rates.html')
hs=h.read_text(encoding='utf-8')
hs=re.sub(r'assets/rates\.js\?v=[^"\']+', 'assets/rates.js?v=20260915-12', hs)
