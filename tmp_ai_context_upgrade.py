...
def money(v):
    s = str(v or "").replace(",", "").replace("원", "").strip()
...
def audit_prepaid():
    saved = json.loads(Path("data/prepaid.json").read_text(encoding="utf-8"))
    saved_map = {p["id"]: {k:p.get(k) for k in ("id","provider_id","name","network","monthly_fee","voice","sms","data")} for p in saved.get("plans", [])}
    live = []
    for pid in PROVIDERS:
        live.extend(source_rows(pid))
    live_map = {p["id"]: p for p in live}
    if saved_map != live_map:
        missing = sorted(set(live_map) - set(saved_map))[:10]
        extra = sorted(set(saved_map) - set(live_map))[:10]
        changed = []
        for k in sorted(set(saved_map) & set(live_map)):
            if saved_map[k] != live_map[k]:
                changed.append((k, saved_map[k], live_map[k]))
                if len(changed) >= 5: break
        raise SystemExit(f"prepaid audit mismatch missing={missing} extra={extra} changed={changed}")
    counts = Counter(p["provider_id"] for p in live)
    ...
    print("prepaid audit ok", dict(counts), "total", len(live))


WORKER = r'''// Git-connected Cloudflare Worker for Woongbi AI
...
export default {
  async fetch(request, env) {
    ...
    try {
      const catalogAnswer = await answerFromPublicCatalog(question);
...
async function answerFromPublicCatalog(question) {
  const q = question.toLowerCase().replace(/\s+/g, " ");
...
def patch_client():
    path = Path("assets/ai-chat.min.js")
    s = path.read_text(encoding="utf-8")
    replacements = [
        ('<button class="wb-ai-chip" type="button">알뜰폰도 되나요?</button>', '<button class="wb-ai-chip" type="button">이 요금제 괜찮나요?</button><button class="wb-ai-chip" type="button">부모님폰 추천해줘</button><button class="wb-ai-chip" type="button">알뜰폰도 되나요?</button>'),
        ('let s=!1;function p(', 'let s=!1,h=[];function p('),
        ('async function l(e){', 'function f(){const e={page:location.pathname,title:document.title};try{const t=document.querySelector(".rate-panel.active");if(t){e.active_tab=t.dataset.panel||"";const a=e=>{const t=document.getElementById(e);if(!t)return"";if("SELECT"===t.tagName){const e=t.options[t.selectedIndex];return e&&e.value?e.textContent.trim():""}return(t.textContent||"").trim()},i={mobile:["carrier","join-type","device-select","plan-select","monthly-total","calc-summary"],studyphone:["studyphone-plan","studyphone-total","studyphone-summary"],mvno:["mvno-provider","mvno-plan","mvno-network","mvno-total","mvno-detail","mvno-summary"],prepaid:["prepaid-provider","prepaid-plan","prepaid-network","prepaid-total","prepaid-detail","prepaid-summary"],internet:["internet-carrier","internet-product","tv-product","internet-total","internet-detail","internet-summary"]}[e.active_tab]||[],o=i.map(a).filter(e=>e&&"—"!==e);e.selection=o.slice(0,5).join(" · ").slice(0,700),e.summary=o.slice(5).join(" · ").slice(0,500)}else e.active_tab="home"}catch(e){}return e}async function l(e){'),
        ('p(e,"user"),n.value="",s=!0,r.disabled=!0;', 'p(e,"user"),h.push({role:"user",content:e}),h=h.slice(-8),n.value="",s=!0,r.disabled=!0;'),
        ('o=setTimeout(()=>a.abort(),2e4);', 'o=setTimeout(()=>a.abort(),14e3);'),
        ('body:JSON.stringify({question:e})', 'body:JSON.stringify({question:e,history:h.slice(-8),context:f()})'),
        ('p(o.answer||"답변을 불러오지 못했습니다. 매장으로 문의해주세요.","bot")}', 'const u=o.answer||"답변을 불러오지 못했습니다. 매장으로 문의해주세요.";p(u,"bot"),h.push({role:"assistant",content:u}),h=h.slice(-8)}'),
        ('p("안녕하세요 👋 웅비통신 AI 안내입니다. 영업시간, 위치, 휴대폰·알뜰폰·인터넷·렌탈 등에 대해 편하게 물어보세요.","bot")', 'p("안녕하세요 👋 웅비통신 AI 안내입니다. 요금제·휴대폰·선불폰·인터넷 조건도 편하게 물어보세요. ‘이 요금제 괜찮나요?’처럼 이어서 질문해도 됩니다.","bot")'),
    ]
    for old, new in replacements:
        if s.count(old) != 1:
            raise SystemExit(f"client patch needle count {s.count(old)} for {old[:60]}")
        s = s.replace(old, new)
...
def patch_html():
    ...
    old = '<script src="assets/ai-chat.min.js?v=20260917-1" defer></script><script src="assets/ai-chat.js?v=20260915-1" defer></script>'
    new = '<script src="assets/ai-chat.min.js?v=20260917-2" defer></script>'
    if old not in s:
        raise SystemExit("index ai script marker missing")
    index.write_text(s.replace(old, new), encoding="utf-8")

    rates = Path("rates.html")
    r = rates.read_text(encoding="utf-8")
    marker = '<script src="assets/rates.min.js?v=20260917-2" defer></script>'
    if marker not in r:
        raise SystemExit("rates script marker missing")
    r = r.replace(marker, marker + '\n  <script src="assets/ai-chat.min.js?v=20260917-2" defer></script>')
    rates.write_text(r, encoding="utf-8")
...
def main():
    ...
    patch_client()
    patch_html()
    print("contextual AI patch complete")
