  ...
  s.onload=function(){
    ...
    if(!document.getElementById('woongbi-news-style')){
      const st=document.createElement('style');st.id='woongbi-news-style';st.textContent=`
        ...
        .news-card{display:flex;flex-direction:column;min-height:210px;background:#fff;border:1px solid var(--line);border-radius:18px;padding:20px;box-shadow:0 8px 24px rgba(16,60,82,.05);transition:.2s ease}
        ...
        .news-card h3{font-size:1.03rem;line-height:1.48;color:var(--blue);margin:0}.news-card p{font-size:.86rem;line-height:1.55;color:var(--muted);margin:10px 0 0}.news-open{margin-top:auto;padding-top:16px;color:var(--teal);font-weight:900;font-size:.84rem}
        .news-empty{grid-column:1/-1;background:#fff;border:1px solid var(--line);border-radius:18px;padding:22px;color:var(--muted)}
        @media(max-width:820px){.news-grid{grid-template-columns:1fr}.news-head{align-items:flex-start;flex-direction:column;gap:8px}.news-card{min-height:auto}}
